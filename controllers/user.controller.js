...
if (!errors.isEmpty()) {
    ...
    try{
        ...
        if(filter_users){  
            if(bcrypt.compareSync(req.body.password, filter_users.password)){
                req.body.new_password = bcrypt.hashSync(req.body.new_password, Number.parseInt(authConfig.rounds));
                const data_user_update = await Users.update({password: req.body.new_password}, {where:{id : id}});
                ...
                return res.status(400).send({
                    data : {},
                    message: [{ msg: 'Contraseña incorrecta' }],
                    success : false,
                });
            }
